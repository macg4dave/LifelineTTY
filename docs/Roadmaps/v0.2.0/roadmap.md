...
| --- | --- | --- | --- | --- |
| **P1 — Config hardening (regression sweep)** | 🟠 In progress | Config | Re-run and extend validation for cols/rows, baud/device overrides, negotiation + wizard defaults; add fixtures + tests for malformed/partial TOML and env overrides in `tests/integration_mock.rs`. | Persist only to `~/.serial_lcd/config.toml`; no new keys unless already documented. Acceptance: fixtures added, integration mock asserts env overrides, smoke covers cols/rows/baud precedence. |
