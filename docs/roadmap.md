...
| **P22** | **Custom character support and built-in icons** _(in progress)_: the icon registry exists, but `display::overlay_icons` still only renders `battery`, `heart`, and `wifi` (others fall back to ASCII) and there is no CGRAM bank manager. Implement runtime glyph loading/scheduling plus docs/tests per Milestone H. |
